...
await new Promise((resolve, reject) => {
  ...
  console.error(error.code === 'EADDRINUSE'
    ? `A porta ${config.port} já está em uso. Pare a outra instância do bot antes de iniciar.`
    : 'Não foi possível iniciar o servidor do bot.');
  process.exit(1);
});

log('INFO', 'http.started');
const publicUrl = process.env.RENDER_EXTERNAL_URL || `http://127.0.0.1:${config.port}`;
console.log(`🌙 Site e bot: ${publicUrl}/#conectar`);

if (config.openBrowser && ['127.0.0.1', 'localhost', '0.0.0.0'].includes(config.host)) {
  openPanel(config.port);
}
